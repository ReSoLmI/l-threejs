<template>
  <!--  Threejs实现汽车3D展示，改变汽车颜色，汽车爆炸分解合体效果-->
  ...
  export default {
    ...
    methods: {
      init() {
        ...
        _GLTFLoader.load(
          ...
          gltf => {
            ...
            console.log('______carModel', _this.carModel)
      ...
      pickupObjects(event) {
        ...
        console.log('________intersects', intersects)
        let model = intersects[0]
        console.log('___model', model)
        if (model) model.object.material.color = new THREE.Color(0xff0000)
      },
      createGUI() {
        let _this = this
