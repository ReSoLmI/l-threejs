...
const resolveSidebarItem = (sidebarConfigItem, pages, base, prefix = '') => {
  ...
  if (Array.isArray(sidebarConfigItem))
    return Object.assign(resolvePageforSidebar(pages, resolve(prefix, sidebarConfigItem[0], base)), {
  ...
  if (children.length === 0 && sidebarConfigItem.path)
    // cover title
    return Object.assign(resolvePageforSidebar(pages, resolve(prefix, sidebarConfigItem.path, base)), {
...
export const getSidebarItems = (page, site, localePath) => {
  ...
  if (page.frontmatter.sidebar === 'auto' || sidebarConfig === 'auto') return resolveSidebarHeaders(page)
