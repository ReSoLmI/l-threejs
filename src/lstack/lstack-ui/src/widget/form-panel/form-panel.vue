<template>
  <rl-close ref="closeRef" action="inject">
    <div v-resize:throttle.initial.150="onResize" ref="panelDom" :style="panelStyle" class="form-panel">
      <lui-step
        v-if="steps"
        ref="stepRef"
        :steps="steps"
...
<script>
  ...
  import { throttle } from '@lstack/utils'
  ...
  export default {
    ...
    data() {
      return {
        ...
        cErrorTip: undefined,
        windowScrollHandleBind: throttle(this.windowScrollHandle.bind(this), 150),
        windowResizeHandleBind: throttle(this.windowResizeHandle.bind(this), 150),
      }
    },
    destroyed() {
      window.removeEventListener('scroll', this.windowScrollHandleBind)
      window.removeEventListener('resize', this.windowResizeHandleBind)
    },
    mounted() {
      window.addEventListener('scroll', this.windowScrollHandleBind)
      window.addEventListener('resize', this.windowResizeHandleBind)
    ...
    methods: {
      windowScrollHandle() {
        this.reSetFooterLeft()
      },
      windowResizeHandle() {
        this.reSetFooterLeft()
      },
      reSetFooterLeft() {
        if (this.$refs.panelDom) {
          const domRect = this.$refs.panelDom.getBoundingClientRect()
          if (this.$refs.footer) {
            this.$refs.footer.$el.style.left = `${domRect.left}px`
          }
        }
      },
      onResize(panelDom) {
        const domRect = panelDom.getBoundingClientRect()
        this.submitBoxStyle.left = `${domRect.left}px`
