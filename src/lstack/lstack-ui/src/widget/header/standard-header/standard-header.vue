  ...
  >
    <template v-if="!!title || !!subtitle" #topLeft>
      <rl-close v-if="showCloseButton" bind-el-event action="inject">
        <lui-button class="standard-header__close-button" shape="square" :icon="closeButtonIcon" />
      </rl-close>
    ...
    <template v-if="showRightCloseButton || $scopedSlots.actions" #topRight>
      ...
      <rl-close v-if="showRightCloseButton" bind-el-event action="inject">
        <lui-button class="standard-header__right-close-button" shape="square" icon="lui-icon-close" />
      </rl-close>
    </template>
...
<script>
  ...
  export default {
    ...
    props: {
      ...
      closeButtonType: { type: String, default: 'close' },
    },
    computed: {
      hasSheets() {
      ...
      closeButtonIcon() {
        if (this.pPageStack && this.pPageStack.size === 'Match') {
          return 'lui-icon-arrow-left'
        }
        if (this.closeButtonType !== 'close') {
          return 'lui-icon-arrow-left'
        }
        return 'lui-icon-close'
      },
    },
  }
</script>
