...
<script>
  ...
  export default {
    ...
    props: {
      disableBlurAfterClick: { type: Boolean, default: false },
      text: String,
    ...
    methods: {
      ...
      handleClick(evt) {
        // button 原生 click 事件会在获取到 dom 的点击事件之后，同时监听 keydown.enter 事件， 将回车事件绑定 click 事件，解决方式是：取消当前按钮聚焦。默认开启
        if (this.tag === 'button' && !this.disableBlurAfterClick) {
          this.blurButton(this.$refs.componentRef)
        }
