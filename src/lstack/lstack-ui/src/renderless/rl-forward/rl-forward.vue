<script>
  ...
  export default {
    ...
    props: {
      // 如果你要使用 openDrawer 的话，请在 main.js 文件中挂载这个 $openDrawer,不挂就报错，同理 popup
      // Vue.prototype.$openDrawer = app.$children[0].apiHandlerInstance.openDrawer
      action: {
        type: String,
        default: 'push',
        validator(val) {
          return (
            ['default', 'popup', 'openPage', 'openDrawer', 'openTab', 'push', 'replace', 'back'].indexOf(val) !== -1
          )
        },
      },
    ...
    methods: {
      ...
      doOpenDrawer() {
        // 如果你要使用的话，请在 main.js 文件中挂载这个 $openDrawer,不挂就报错，同理 popup
        if (isFunction(this.$openDrawer)) {
          this.$openDrawer(this.cTo(), this.config)
        }
      },
      doForward() {
        switch (this.action) {
