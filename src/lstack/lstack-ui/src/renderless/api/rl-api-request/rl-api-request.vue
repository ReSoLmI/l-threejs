<script>
  ...
  export default {
    ...
    props: {
      ...
      interceptCommonErrorHandler: [Boolean, Function],
    ...
    methods: {
      ...
      mValidBeforeRequest() {
        ...
        return this.requestConfig || (this.cRequestUrlValid() && this.cRequestQueryValid() && this.cRequestBodyValid())
      },
      request() {
          ...
          if (this.mValidBeforeRequest()) {
            ...
            this.mInvokeRequest()
              .then(response => {
                ...
                if (!this.cProcessResponse(response, this.responseData, this.responseBizData)) {
                  resolve(response)
                }
      ...
      cProcessResponse(response, responseData, responseBizData) {
        return this.cHandleResponse(response, responseData, responseBizData)
      },
      aDefaultErrorHandler(error) {
      ...
      iDefaultErrorHandler(error) {
        return false
      },
      cDefaultErrorHandler(error) {
        return this.iDefaultErrorHandler(error) || this.aDefaultErrorHandler(error)
      },
      aCommonErrorHandler(error) {
        const { $LUI: opts = {} } = this
        const { requestCommonErrorHandler } = opts
        if (isFunction(requestCommonErrorHandler)) {
          return requestCommonErrorHandler(error)
        }
        return false
      },
      iCommonErrorHandler(error) {
        return false
      },
      cCommonErrorHandler(error) {
        return this.iCommonErrorHandler(error) || this.aCommonErrorHandler(error)
      },
      iHandleError(error) {
        return false
      },
      ...
      cProcessError(error) {
        const interceptCommonErrorHandler = isFunction(this.interceptCommonErrorHandler)
          ? this.interceptCommonErrorHandler(error)
          : this.interceptCommonErrorHandler

        let consumed
        if (!interceptCommonErrorHandler) {
          consumed = consumed || this.cCommonErrorHandler(error)
        }
        return consumed || this.cHandleError(error) || this.cDefaultErrorHandler(error)
      },
      iHandleCancel(error) {},
      aHandleCancel(error) {},
      cHandleCancel(error) {
