          ...
          <lui-col v-if="isNotEmptyArray(keys)" :span="isNotEmptyArray(subKeys) ? 10 : 24">
            <lui-form-item v-for="(key, index) in keys" :key="index" :no-margin="index === 0" :label="kvMap[key]">
              <slot name="infoItemValue" v-bind="{ key, index, kvMap, info }">
                <el-tooltip v-if="key === tooltipKey" effect="dark" :content="info[key] || '--'" placement="top">
                  <div style="overflow: hidden; text-overflow: ellipsis">{{ info[key] || '--' }}</div>
                </el-tooltip>
                <lui-text v-else inline>
                  {{ info[key] }}
                </lui-text>
              </slot>
            </lui-form-item>
...
<script>
  ...
  export default {
    ...
    props: {
      ...
      tooltipKey: String, // 内容过长的信息通过 tooltip 来显示
    },
    methods: {
      isNotEmptyArray,
    },
